...
/*방법1*/
select sc_start_time from movie 
	join screen on mo_num=sc_mo_num
	where mo_title like concat("%","컨저링3","%") and sc_date like "2021-06-03";

/*방법2*/
select sc_start_time from screen
	where sc_mo_num = (select mo_num from movie where mo_title like concat("%","컨저링3","%") 
		and sc_date like "2021-06-03");  
    

 /* 컨저링3의 CGV청주성안길 지점의 상영 정보를 확인하는 쿼리문*/
/*방법1*/-- screen.* 하면 screen에 대한 정보만 나옴 / join
select screen.* from screen
	join screen_room on sc_sr_num = sr_num
    join movie on sc_mo_num = mo_num
	where sr_th_branch like "CGV청주성안길" and mo_title like concat("%","컨저링3","%");
    
/*방법2*/
select screen.* from screen
	where sc_sr_num = (select sr_num from screen_room where sr_th_branch like 'CGV청주성안길')
    and sc_mo_num = (select mo_num from movie where mo_title like concat('%','컨저링3','%'));
    
/* 청주 성안길 지점의 1관의 예매 가능한 좌석 정보를 확인하는 쿼리문*/
/*방법1*/
select se_num, se_name from seat
	join screen_room on se_sr_num=sr_num 
    join reservation on se_num not in (select re_se_num from reservation)
	where re_sr_name like '1관' and sr_th_branch like 'CGV청주성안길';


/*방법2*/
select se_num, se_name from seat
	where se_sr_num = (select sr_num from screen_room where sr_th_branch like "CGV청주성안길" and sr_name like "%1관%") 
    and se_num not in (select re_se_num from reservation);
