...
public class day9_CardGame2 {
	...
	public static void main(String[] args) {	
		...
		int []arr = new int[] {1,1,4,5,7,8,9};
		
		int result=0;
		int [] resultArr;

		if((result = poker(arr)) != 0) {
			System.out.print(poker(arr)+"포커 입니다");
		}else if((result = fullHouse(arr)) != 0) {
			System.out.print(fullHouse(arr)+"풀하우스 입니다.");
		}else if((result = straight(arr)) != 0) {
			System.out.print(straight(arr)+ "스트레이트 입니다.");
		}else if((result = triple(arr)) != 0) {
			System.out.print(triple(arr)+"트리플 입니다.");
		}else if((resultArr=twoPair(arr)) != null) {
			for(int tmp : resultArr) {
				System.out.print(tmp + " ");
			}System.out.print("투페어 입니다.");
		}else if((result = onePair(arr)) != 0) {
			System.out.print(onePair(arr) + "원페어 입니다.");
		}else {
			System.out.print(arr[arr.length-1]+"탑입니다.");
		}
...
//		}
		...
		System.out.println("풀하우스의 숫자: " + fullHouse(arr));
		
		System.out.println("스트레이트의 가장 높은 숫자: " + straight(arr));
...
/*		for(i=0; i<card.length-1; i+=1) {	//2개의 수씩 비교해서 card.length-1 인데 (현재1,2,3,4,5,6,7 배열은 7개짜리라ㅓ -1인데 <작아서 5까지가 i의 최대임
		...
		if(count >=5) {	// 마지막까지 연속으로 끝난 경우는 위에서 잡히지 않아서 밑에 한번 더 확인해줘야 함.
			return num;
		}
		return 0;
	}
